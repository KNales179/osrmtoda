...
router.post('/book', async (req, res) => {
  try {
    ...
    console.log("📨 Booking request received from passenger:", passengerName);

    // STEP 1: Get all online drivers
    const onlineDrivers = await DriverStatus.find({ isOnline: true });

    if (onlineDrivers.length === 0) {
      console.log("❌ No available drivers right now.");
    ...
    if (!nearestDriver) {
      console.log("❌ No nearby driver found.");
      return res.status(404).json({ message: "No nearby driver found." });
    }

    console.log("👤 Matched driver name:", nearestDriver.driverName);
    ...
    return res.status(200).json({
        ...
        matchedDriverName: nearestDriver.driverName, 
